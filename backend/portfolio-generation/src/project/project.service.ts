...
export class ProjectService {
  ...
  generatePortfolio(data: PortfolioCreateDto): PortfolioDto[] {
    ...
    let portfolios: PortfolioDto[] = projects.map((project) => ({
      project,
      tons: 0,
    }));
    let shortfall = 0;
    ...
    while (true) {
      ...
      for (let index = 0; index < portfolios.length; index++) {
        ...
        const tonsForPortfolioWantToAssign =
          tonsToAssign * portfolio.project.distributionWeight;
        const tonsForPortfolioWavailableToAssign =
          portfolio.project.offeredVolumeInTons - portfolio.tons;
        if (tonsForPortfolioWavailableToAssign > 0) {
          // Can still assign more tons to this project/portfolio
          if (
            tonsForPortfolioWantToAssign > tonsForPortfolioWavailableToAssign
          ) {
            ...
            portfolio.tons =
              portfolio.tons + tonsForPortfolioWavailableToAssign;
            shortfall =
              tonsForPortfolioWantToAssign - portfolio.tons + shortfall;
          } else {
            // Here we can assign while still having spares
            totalPercentagesForRedistrbution +=
              portfolio.project.distributionWeight;
      ...
      if (shortfall === 0 || this.checkIfAllFullyAssigned(portfolios)) {
        ...
        portfolios = this.redistrbutePercentage(
          portfolios,
          totalPercentagesForRedistrbution,
        );
      }
  ...
  private redistrbutePercentage(
    portfolios: PortfolioDto[],
    totalPercentagesForRedistrbution: number,
  ): PortfolioDto[] {
